...
class sketch2pic():
    ...
    def __init__(self, name, lambda_=100, generator_activation='tanh'):
        ...
        predictions = self.dual_discriminator([concat_real, concat_fake])
        ...
        )  # we don't need metrics because we only need the losses for metrics
    ...
    def train(self, img_pairs, dev_pairs, batch_size=4, epochs=15, \
        ...
        def perform_batch_train(batch):
            true_photos = batch[0]
            sketches = batch[1]
            weights_before_train = self.dual_discriminator.get_weights()
            cGAN_metrics = self.cGAN.train_on_batch(
                [sketches, true_photos],
                [true_photos, np.ones(sketches.shape[0]), np.zeros(sketches.shape[0])])
            self.dual_discriminator.set_weights(list(
                2 * np.array(weights_before_train) - np.array(self.dual_discriminator.get_weights())
                ))
        ...
        def log_metrics(train_batch, dev_batch, checkpoint, return_fakes=True):
            ...
            train_loss = cGAN_train_metrics[0] + cGAN_train_metrics[1] + cGAN_train_metrics[2]
            dev_loss = cGAN_dev_metrics[0] + cGAN_dev_metrics[1] + cGAN_dev_metrics[2]
    ...
    def save_weights(self, filename_base=''):
        ...
        self.dual_discriminator.save_weights(filename_base + '_disc_weights.h5')
        self.cGAN.save_weights(filename_base + '_cGAN_weights.h5')

    def load_weights(self, filename_base):
        self.dual_discriminator.load_weights(filename_base + '_disc_weights.h5')
